...
class Elastic extends SculptBase {
  ...
  getFallOff(dist) {
    ...
    var baseFalloff = SculptBase.prototype.getFallOff.call(this, dist);
    return Math.pow(baseFalloff, this._elasticity);
  }
  ...
  sculptStroke() {
    ...
    this.elastic(picking.getPickedVertices(), this._moveData.center, picking.getLocalRadius2(), this._moveData, picking);

    if (useSym) {
      this.updateMoveDir(pickingSym, mouseX, mouseY, true);
      this.elastic(pickingSym.getPickedVertices(), this._moveDataSym.center, pickingSym.getLocalRadius2(), this._moveDataSym, pickingSym);
    }
  ...
  elastic(iVerts, center, radiusSquared, moveData, picking) {
    var mesh = this.getMesh();
    ...
    var eps = radius;
    var eps2 = radiusSquared;

    // Map elasticity (0.1 to 3.0) to Poisson's ratio nu (0.40 to 0.499)
    // 0.40 = moderate volume preservation (more stretchy)
    // 0.499 = high volume preservation (rubber/jello)
    var nu = 0.40 + 0.099 * (this._elasticity - 0.1) / 2.9;

    // Elastic constants (assuming shear modulus mu = 1.0)
    var a = 1.0;
    var b = 3.0 - 4.0 * nu;

    // Normalization factor at r = 0 to ensure the center moves exactly with the cursor
    var S = (1.5 * a - b) / eps;
    ...
    for (var i = 0, l = iVerts.length; i < l; ++i) {
      ...
      var r = Math.sqrt(r2);
      var r_eps = Math.sqrt(r2 + eps2);
      var r_eps3 = r_eps * r_eps * r_eps;

      // Dot product r . f (where f is the drag direction vector)
      var dot_r_f = dx * dirx + dy * diry + dz * dirz;

      // Regularized Kelvinlets coefficients
      var k1 = (a - b) / r_eps + (a * eps2) / (2.0 * r_eps3);
      var k2 = b / r_eps3;

      // Displacement vector components
      var ux = k1 * dirx + k2 * dot_r_f * dx;
      var uy = k1 * diry + k2 * dot_r_f * dy;
      var uz = k1 * dirz + k2 * dot_r_f * dz;

      // Normalize & apply spatial falloff + alpha texture masking
      var dist = r / radius;
      var fallOff = this.getFallOff(dist);
      fallOff *= mAr[ind + 2] * picking.getAlpha(vx, vy, vz, this._focalShiftFalloff ? this._focalShift : 0);

      vAr[ind] += (ux / S) * fallOff;
      vAr[ind + 1] += (uy / S) * fallOff;
      vAr[ind + 2] += (uz / S) * fallOff;
    }
  }
